...
__global__ void 1Dconv(int *outputArray, int *inputArray, int arg, int length)
{
	int x, i;
	__shared__ int shared_array[BLOCKSIZE+2];
	int kernel1D[3][3] = {1,1,1,0,0,0,1,0,1};
...
__global__ void 2Dconv(int *outputImage, int *inputImage, int arg, int width, int height)
{
	int i, j, k, pixel;
...
void do_2D_conv(char **argv)
{
	...
	2Dconv <<< dimGrid, dimBlock >>> (deviceOutput, deviceInput, arg, imageWidth, imageHeight);
